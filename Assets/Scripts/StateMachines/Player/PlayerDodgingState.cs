...
public class PlayerDodgingState : PlayerBaseState {
  ...
  public override void Tick(float deltaTime) {
    ...
    forward = stateMachine.DodgeLength / stateMachine.DodgeDuration * camForward * dodgingDirectionInput.y;
    right =  stateMachine.DodgeLength / stateMachine.DodgeDuration * camRight * dodgingDirectionInput.x;
    forward.y = 0f;
    right.y = 0f;

    Move((right + forward) * stateMachine.FreeLookMovementSpeed, deltaTime);
